...
/**
 * @file DacAdvantech.cpp
 * @brief Dac Advantech source file.
 */
#ifdef ADVANTECHDAC
...
void 
DacAdvantech::setDbPath(QString _path){
...
bool
DacAdvantech::connectDac(QString newdac) {
  ...
  for (int i = 0 ; i < dac.size(); i++)  {
	...
	//
	// Free ADVANTECHDAC related structures
	//
	if (fd.at(i))   {
	  free(fd.at(i));
	  fd.remove(i);
  ...
  // First time DacAdvantech connection
...
DacAdvantech::setDacValue(QString newdac, int output, double value) {
  ...
  for (int index = 0 ; index < dac.size(); index++)  {
    if ( dac.at(index) == newdac && connectSuccess.at(index) == true) {
      QLOG_DEBUG ( ) << "DacAdvantech::setDacValue " << newdac << ": " << output << ":" << value;
      // Assign DacAdvantech value to ouptut
      // enable sync
      status = DRV_EnableSyncAO(*(fd.at(index)), 1);
      if (status) {
	DRV_GetErrorMessage(status, err_msg);
